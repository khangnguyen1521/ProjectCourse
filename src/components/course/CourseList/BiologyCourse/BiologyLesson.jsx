...
const BiologyLesson = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/courses')
      ...
      .then(data => {
        if (data.length > 3) {
          setLessons(data[3].lessons);
          const foundLesson = data[3].lessons.find(l => l.id === lessonId);
  ...
  if (!lesson) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <Link to="/courses/4" className="text-blue-500 hover:text-blue-700 mt-4 inline-block">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <LessonLayout
        ...
        onPrev={() => {
          if (currentIndex > 0) navigate(`/courses/4/${lessons[currentIndex - 1].id}`);
        }}
        onNext={() => {
          if (currentIndex < lessons.length - 1) navigate(`/courses/4/${lessons[currentIndex + 1].id}`);
        }}
      >
        <div className="mb-8">
          ...
          <div className="text-gray-500 text-sm mb-2">
            <Link to="/courses/4" className="hover:underline text-violet-600">Sinh học</Link>
