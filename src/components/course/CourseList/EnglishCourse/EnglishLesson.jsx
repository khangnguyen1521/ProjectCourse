...
const EnglishLesson = () => {
  ...
  useEffect(() => {
    fetch(${API_ENDPOINTS.COURSES})
      ...
      .then(data => {
        if (data.length > 8) {
          setLessons(data[8].lessons);
          const foundLesson = data[8].lessons.find(l => l.id === lessonId);
  ...
  if (!lesson) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        ...
        <Link to="/courses/9" className="text-blue-500 hover:text-blue-700 mt-4 inline-block">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <LessonLayout
        ...
        onPrev={() => {
          if (currentIndex > 0) navigate(`/courses/9/${lessons[currentIndex - 1].id}`);
        }}
        onNext={() => {
          if (currentIndex < lessons.length - 1) navigate(`/courses/9/${lessons[currentIndex + 1].id}`);
        }}
      >
        <div className="mb-8">
          ...
          <div className="text-gray-500 text-sm mb-2">
            <Link to="/courses/9" className="hover:underline text-violet-600">Tiếng Anh</Link>
